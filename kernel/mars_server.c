			  ...
			  struct lamport_time *check_prosumer_epoch)
{
	...
	prosumer_epoch = ordered_readlink(prosumer_epoch_path,
					  &old_prosumer_epoch);
	MARS_DBG("prosumer '%s' -> '%s' peer '%s' epoch old=%ld.%09ld new=%ld.%09ld\n",
		 prosumer_epoch_path,
		 prosumer_epoch,
		 peer,
		 old_prosumer_epoch.tv_sec,
	...
	if (check_prosumer_epoch &&
		...
		MARS_ERR("prosumer '%s' -> '%s' peer '%s' epoch violation %ld.%09ld < %ld.%09ld\n",
			 prosumer_epoch_path,
			 prosumer_epoch,
			 peer,
			 check_prosumer_epoch->tv_sec,
