...
static void client_ref_io(struct client_output *output, struct mref_object *mref)
{
	...
	mref_a->submit_stamp = get_real_lamport();
		...
		case CMD_GETINFO:
		{
			...
			if (this_info.stor_state.stor_epoch.tv_sec) {
				if (output->stor_epoch.tv_sec &&
					...
					mutex_unlock(&output->mutex);
					MARS_ERR("stor_epoch mismatch %ld.%09ld != %ld.%09ld\n",
						 this_info.stor_state.stor_epoch.tv_sec,
						 this_info.stor_state.stor_epoch.tv_nsec,
						 output->stor_epoch.tv_sec,
...
void _do_timeout(struct client_output *output, struct list_head *anchor, int *rounds, bool force)
{
	...
	timeout_stamp = get_real_lamport();
